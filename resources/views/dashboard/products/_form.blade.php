...
<div class="space-y-6 max-w-6xl mx-auto">
    <x-molecules.dashboard.forms.form-section title="Informasi Produk">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <x-molecules.dashboard.forms.form-input label="Nama Produk" name="name" :value="$product?->name" placeholder="Nama produk" required data-slug-source="slug" />
            <x-molecules.dashboard.forms.form-input label="Slug" name="slug" :value="$product?->slug" placeholder="auto-generated" required />
            <x-molecules.dashboard.forms.form-input type="select" label="Kategori" name="product_category_id" :value="$product?->product_category_id"
                :options="$categories ?? []" />
            <x-molecules.dashboard.forms.form-input type="number" label="Harga" name="price" :value="$product?->price" placeholder="0" required />
        </div>
        <x-molecules.dashboard.forms.form-input label="No. Telepon Pemesanan" name="phone_number" :value="$product?->phone_number" placeholder="+6281234567890" />
        <x-molecules.dashboard.forms.form-input type="toggle" name="is_available" :value="$product?->is_available ?? true" placeholder="Produk tersedia" />
        <x-molecules.dashboard.forms.form-input type="textarea" label="Deskripsi" name="description" :value="$product?->description" :rows="4" />
        <x-molecules.dashboard.forms.form-input type="textarea" label="Teks Cara Order" name="order_text" :value="$product?->order_text" :rows="4"
            placeholder="Contoh: Hubungi admin via WhatsApp..." />
    </x-dashboard.form-section>

    <x-molecules.dashboard.forms.form-section title="Gambar Produk" description="Tambah atau kelola gambar produk. Gambar pertama secara default menjadi gambar utama.">
        <div id="imageRepeater" class="space-y-3" data-count="{{ ($product?->images?->count() ?? 0) }}">
...
@vite(['resources/js/dashboard/slug-helper.js', 'resources/js/dashboard/product-form.js'])
