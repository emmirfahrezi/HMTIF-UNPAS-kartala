<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-6" x-data="slugHelper(@js(old('name', $category?->name)), @js(old('slug', $category?->slug)))">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-molecules.shared.forms.form-input 
                        label="Nama Kategori" 
                        name="name" 
                        placeholder="Contoh: Berita, Akademik, dll."
                        :value="$category?->name ?? ''" 
                        required 
                        x-model="sourceValue" />
    ...
    <div class="space-y-8">
        {{-- Info Card --}}
        <div class="bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl p-6 border border-indigo-100 dark:border-indigo-500/20 relative overflow-hidden transition-colors duration-300">
             <div class="absolute top-0 right-0 p-4 opacity-[0.05] text-indigo-600 pointer-events-none">
                <x-heroicon-o-information-circle class="size-20" />
            </div>
            <div class="flex gap-4 relative z-10">
                <div class="size-10 rounded-xl bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 flex items-center justify-center shrink-0">
                    <x-heroicon-s-light-bulb class="size-5" />
                </div>
                <div>
                    <h4 class="text-xs font-black text-indigo-600 uppercase tracking-widest mb-1">Tips</h4>
                    <p class="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                        Gunakan kategori untuk mengelompokkan pengumuman agar mahasiswa lebih mudah memfilter informasi.
                    </p>
                </div>
            </div>
        </div>
