...
<x-layouts.dashboard pageTitle="Profil Saya" :breadcrumbs="[['label' => 'Profil Saya']]">
    <div x-data="{
        ...
        {{-- Form Utama Profil --}}
        <form action="{{ route('dashboard.profile.update') }}" method="POST" enctype="multipart/form-data" @submit="submitting = true" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            @method('PUT')

            {{-- Kolom Kiri: Foto, Info Singkat, & Tombol Kontrol --}}
            <div class="space-y-6">
                <div class="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col items-center text-center relative overflow-hidden transition-all duration-300">
                    {{-- Decorative Background Circle --}}
                    <div class="absolute -top-12 -right-12 size-32 bg-primary/5 rounded-full blur-2xl"></div>

                    {{-- Avatar Display / Uploader --}}
                    <div class="relative group mt-4">
                        <div class="size-32 rounded-3xl overflow-hidden ring-4 ring-primary/10 border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex items-center justify-center transition-all duration-300">
                        ...
                        <label x-show="editMode" x-cloak
                            ...
                            <input type="file" name="photo" accept="image/*" class="hidden" @change="onAvatarChange">
                        </label>
                    </div>

                    {{-- User Basic Info --}}
                    <h3 class="mt-5 text-lg font-bold text-slate-850 dark:text-white" x-text="name"></h3>
                    <p class="text-xs text-slate-400 dark:text-slate-500 font-medium">{{ $email }}</p>
                    
                    {{-- Badge Role --}}
                    <span class="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-primary/10 text-primary border border-primary/10">
                    ...
                    <div class="w-full space-y-3">
                        ...
                        <x-atoms.shared.button 
                            type="submit" 
                            variant="primary" 
                            class="w-full py-3 shadow-lg shadow-primary/20" 
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <x-molecules.shared.forms.form-section 
                    ...
                    description="Kelola nama profil dan detail dasar keanggotaan Anda" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <x-molecules.shared.forms.form-input 
                            type="text" 
                            label="Jabatan Pengurus (Read Only)" 
                            name="position" 
                            value="{{ $position }}" 
                            class="disabled:opacity-60 disabled:cursor-not-allowed" 
                            disabled 
                ...
                <x-molecules.shared.forms.form-section 
                    title="Sosial Media & Tautan" 
                    description="Hubungkan profil pengurus Anda dengan jejaring luar" 
