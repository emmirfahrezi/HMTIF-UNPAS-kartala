@php
    ...
    $statusClasses = [
        ...
        'planned' => 'border-slate-200 bg-white/70 text-slate-400',
    ];
@endphp
            ...
            <div class="mx-auto flex max-w-screen-2xl flex-col gap-8 px-6 py-12 lg:flex-row lg:items-start lg:justify-between lg:px-8 lg:py-16">
                <div class="max-w-3xl">
                    ...
                    <h1 class="mt-3 text-4xl font-black text-slate-950 md:text-5xl">
                        {{ data_get($content, 'title', 'Tim Pengembang') }}
                    </h1>
        ...
        <section class="bg-white py-12 lg:py-16">
            <div class="mx-auto max-w-screen-2xl px-6 lg:px-8">
                <div class="mb-8 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                    <div>
                        <h2 class="text-2xl font-black text-slate-950 md:text-3xl">Anggota Tim ({{ $activePeriod }})</h2>
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    @forelse($members as $member)
                        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-primary/30 hover:shadow-lg hover:shadow-slate-200/70">
                            <img src="{{ data_get($member, 'photo') }}" alt="{{ data_get($member, 'name') }}"
                                class="size-14 rounded-2xl border border-slate-200 object-cover">
                            <h3 class="mt-4 text-base font-black text-slate-950">{{ data_get($member, 'name') }}</h3>
                            <p class="mt-1 text-sm font-bold text-primary">{{ data_get($member, 'role') }}</p>
            ...
            <div class="mx-auto max-w-screen-2xl px-6 lg:px-8">
                ...
                <div class="relative mx-auto mt-10 max-w-4xl space-y-7 before:absolute before:left-4 before:top-2 before:h-full before:w-px before:bg-slate-300 md:before:left-1/2">
                    @forelse($milestones as $index => $milestone)
                        @php
                            $status = data_get($milestone, 'status', 'planned');
                            $isRight = $index % 2 === 1;
                        @endphp
                        <article class="relative grid grid-cols-1 gap-5 md:grid-cols-2">
                            <span class="absolute left-4 top-6 z-10 size-3 -translate-x-1/2 rounded-full border-2 border-white {{ $status === 'current' ? 'bg-primary ring-4 ring-primary/15' : 'bg-slate-400' }} md:left-1/2"></span>
                            <div class="{{ $isRight ? 'md:col-start-2 md:pl-10' : 'md:pr-10' }} pl-10 md:pl-0">
                                <div class="rounded-2xl border p-5 shadow-sm {{ $statusClasses[$status] ?? $statusClasses['planned'] }}">
                                    <div class="mb-3 flex flex-wrap items-center gap-2">
                                        <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black uppercase text-slate-500">
                                            Periode {{ data_get($milestone, 'period') }}
                                        </span>
                                        <span class="rounded-full bg-white px-2.5 py-1 text-[10px] font-black uppercase text-primary shadow-sm">
                                            {{ $statusLabels[$status] ?? 'Rencana' }}
                                        </span>
                                    </div>
                                    <h3 class="text-base font-black text-slate-950">{{ data_get($milestone, 'title') }}</h3>
                                    <p class="mt-2 text-sm font-medium leading-6 text-slate-600">{{ data_get($milestone, 'description') }}</p>
                                </div>
                            </div>
                        </article>
                    @empty
