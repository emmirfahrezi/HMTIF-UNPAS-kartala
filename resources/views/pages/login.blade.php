    ...
    <main class="min-h-screen flex items-center justify-center bg-slate-50 relative overflow-hidden py-12 px-4">
        ...
        <div class="w-full max-w-[440px] relative">
            ...
            <div x-data="{
                ...
                <form action="{{ route('login.store') }}" method="POST" class="space-y-6" @submit="submitting = true" novalidate>
                    @csrf

                    {{-- Email Field --}}
                    <x-molecules.pages.forms.form-field id="email_address" label="Email Address" :required="true">
                        <input
                            id="email_field" name="email" type="email" autocomplete="username"
                            inputmode="email" placeholder="nama@email.com" required
                            value="{{ old('email') }}"
