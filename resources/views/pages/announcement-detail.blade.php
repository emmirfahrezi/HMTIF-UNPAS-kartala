<x-layouts.app
    :title="$announcement->title . ' | HMTIF UNPAS'"
    :description="$announcement->excerpt ?: \Illuminate\Support\Str::limit(strip_tags($announcement->body), 160)"
...
>
    @php
        ...
        $relatedAnnouncements = \App\Models\Announcement::with('category')
            ...
            ->when($announcement->announcement_category_id, fn ($query) => $query->where('announcement_category_id', $announcement->announcement_category_id))
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-10 items-start">
                <div>
                    <div class="flex flex-wrap items-center gap-3 mb-5">
                        <span class="inline-flex items-center px-3 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-black uppercase tracking-[0.2em]">
                ...
                <div class="rounded-[2rem] overflow-hidden border border-slate-200 shadow-lg bg-slate-50">
                    <img src="{{ $heroImage }}" alt="{{ $announcement->title }}" class="w-full h-full object-cover" data-fallback-src="{{ asset('images/placeholders/announcement.svg') }}">
                </div>
            </div>
        </div>
    </section>

    <section class="py-16 bg-section/30 min-h-screen">
        <div class="mx-auto px-6 lg:px-8 max-w-screen-2xl">
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-10">
                <article class="bg-white rounded-[2rem] border border-slate-200 p-8 md:p-10 shadow-sm">
                    <h2 class="text-2xl font-black italic uppercase tracking-tighter text-heading mb-6">Isi Pengumuman</h2>
                    ...
                    @if ($announcement->file)
                        <div class="mt-8 pt-6 border-t border-slate-100">
                            <a href="{{ $announcement->file }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 text-primary font-bold hover:underline">
                ...
                <aside class="space-y-6">
                    <div class="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm">
                        <h3 class="text-sm font-black uppercase tracking-[0.2em] text-slate-400 mb-5">Pengumuman Terkait</h3>
                        <div class="space-y-4">
                            @forelse ($relatedAnnouncements as $item)
                                <a href="{{ route('announcements.show', $item->slug) }}" class="block rounded-2xl border border-slate-100 p-4 hover:border-primary/20 hover:bg-slate-50 transition-all">
