...
{{-- Hero Section with Breadcrumb (Left Aligned) --}}
<section class="relative pt-20 pb-6 overflow-hidden bg-white">
    ...
    <div class="container mx-auto px-6 relative z-10">
        <div class="flex flex-col items-start gap-4 mb-8">
            ...
            <span
                class="px-4 py-1.5 rounded-full bg-primary/10 text-primary text-[10px] font-black uppercase tracking-[0.2em]">{{ $statusLabel }}
                • Program Kerja</span>
        </div>

        <h1
            class="text-2xl md:text-4xl font-black text-heading mb-6 uppercase italic tracking-tighter leading-tight max-w-5xl">
            {{ $activity?->title ?? 'Detail Kegiatan HMTIF' }}
        </h1>
        ...
        <div class="flex flex-wrap gap-6 text-sm font-medium text-body/60">
            <div class="flex items-center gap-2">
                ...
                Jadwal:
                {{ $activity?->start_date?->translatedFormat('d F Y') ?? 'Tanggal belum tersedia' }}
            </div>
            <div class="flex items-center gap-2">
