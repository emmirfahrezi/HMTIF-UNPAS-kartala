...
<div {{ $attributes->merge(['class' => 'bg-white p-8 rounded-2xl border border-border shadow-sm hover:shadow-xl transition-all duration-700 hover:-translate-y-2 relative overflow-hidden reveal reveal-up']) }}>
    ...
    <div class="flex flex-col gap-2 relative z-10 text-left">
        <span class="text-4xl md:text-5xl font-extrabold text-primary group-hover:scale-110 transition-transform duration-300 inline-block origin-left">
            {{ $value }}
        </span>
        <h4 class="text-lg font-bold text-heading uppercase tracking-widest border-l-4 border-primary pl-3">
            {{ $label }}
        </h4>
        <p class="text-sm text-body mt-2 leading-relaxed opacity-60 group-hover:opacity-100 transition-opacity duration-300">
