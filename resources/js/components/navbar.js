export function initNavbar() {
  ...
  const navElements = topNav
    ? topNav.querySelectorAll('a, button, h1, span')
    : [];
  const logo = document.getElementById('nav-logo');

  // Ambil status awal dari data-attribute
  const isTransparentInitial =
    navbar.getAttribute('data-transparent') === 'true';
  ...
  if (isTransparentInitial) {
    const updateNavbar = () => {
      if (window.scrollY > 50) {
        ...
        if (logo)
          logo.classList.remove('filter', 'grayscale', 'brightness-200');
